...
namespace OpenTracker
{
    class Hands
    {
            ...
            if (idSession > 0)
            {
                if (handFilter == "")
                    ...
                    dr = sql.Select("SELECT C.IDHAND, C.CARD, HP.NETWON " +
                                    ...
                                    "WHERE H.TYPE='C' AND P.IDPLAYER = " + idPlayer + " AND HS.IDSESSION = " + idSession + " AND C.IDHAND IN (" +
                                    subquery +
                                    ") ORDER BY C.IDHAND LIMIT 200 OFFSET " + ((page - 1) * 200).ToString());
            }
            else
            {
                if (handFilter == "")
                    ...
                    dr = sql.Select("SELECT C.IDHAND, C.CARD, HP.NETWON " +
                                    ...
                                    "INNER JOIN HANDSESSIONS HS ON HP.IDHAND=HS.IDHAND " +
                                    "INNER JOIN HANDS H ON HP.IDHAND=H.IDHAND " +
                                    "WHERE H.TYPE='C' AND P.IDPLAYER = " + idPlayer + " AND C.IDHAND IN (" +
                                    subquery +
                                    ") ORDER BY C.IDHAND LIMIT 200 OFFSET " + ((page - 1) * 200).ToString());
            }
        ...
        internal int GetMaxPages(int idPlayer, string handFilter, int idSession)
        {
            ...
            {
                if (handFilter == "")
                    dr = sql.Select("SELECT COUNT(*) / 200 TOT, COUNT(*) % 200 REST FROM CARDS C INNER JOIN PLAYERS P ON C.IDPLAYER = P.IDPLAYER INNER JOIN HANDPLAYERS HP ON HP.IDPLAYER=P.IDPLAYER AND HP.IDHAND=C.IDHAND INNER JOIN HANDSESSIONS HS ON HP.IDHAND=HS.IDHAND INNER JOIN HANDS H ON HP.IDHAND=H.IDHAND WHERE H.TYPE='C' AND P.IDPLAYER = " + idPlayer + " AND HS.IDSESSION = " + idSession);
                else
            ...
            {
                if (handFilter == "")
                    dr = sql.Select("SELECT  COUNT(*) / 200 TOT, COUNT(*) % 200 REST FROM CARDS C INNER JOIN PLAYERS P ON C.IDPLAYER = P.IDPLAYER INNER JOIN HANDPLAYERS HP ON HP.IDPLAYER=P.IDPLAYER AND HP.IDHAND=C.IDHAND INNER JOIN HANDS H ON HP.IDHAND=H.IDHAND WHERE H.TYPE='C' AND P.IDPLAYER = " + idPlayer);
                else
                    dr = sql.Select("SELECT  COUNT(*) / 200 TOT, COUNT(*) % 200 REST " +
                                    "FROM CARDS C " +
                                    "INNER JOIN PLAYERS P ON C.IDPLAYER = P.IDPLAYER " +
